...
//    }
     ...
     * @return
     */
    public static int getTotalLenth(Collection<String> strs) {
...
//    }
     ...
     * @return
     */
    public static boolean findCollision(Collection<String> c1, Collection<String> c2) {
     ...
     * @return
     */
    public static SimilarityEnum getListSimilarity(List<String> c1, List<String> c2) {
        if (CollectionUtils.isEmpty(c1) && CollectionUtils.isEmpty(c2)) {
            return SimilarityEnum.BOTHNULL;
        } else if (CollectionUtils.isEmpty(c1) || CollectionUtils.isEmpty(c2)) {
            return SimilarityEnum.ONENULL;
        }

        boolean foundSame = false;
        L1: for (String item1 : c1) {
            for (String item2 : c2) {
        ...
        if (!foundSame) {
            return SimilarityEnum.DIFFERENT; // 不存在相同元素, different
        }

        if (c1.size() != c2.size()) {
            return SimilarityEnum.SIMILAR; //存在相同元素，但元素个数同步, similar
        }

        c1 = new ArrayList<String>(c1); // clone the list, keep origin list un-touched
        c2 = new ArrayList<String>(c2);
        Collections.sort(c1);
        Collections.sort(c2);
        for (int i = 0; i < c1.size(); i++) {
            if (!c1.get(i).equals(c2.get(i))) {
                return SimilarityEnum.SIMILAR; // 存在不同元素
            }
        }

        return SimilarityEnum.IDENTICAL; // 元素完全一致
    }
    ...
    public static List<String> str2List(String str , String separator){
        ...
        List<String> result = Splitter.on(separator).trimResults().splitToList(str);
        return result;
